...
#' @param pca_name String specifying the name to be used to store the result in
#' the input object.
...
quickMNN.SingleCellExperiment <- function(object, block,
    ...
    lognorm_args$mode <- mode %||% lognorm_args$mode
    ...
    runMNN(
        ...
        dimred = pca_name, ..., threads = threads
    )
}

#' @export
#' @rdname quickMNN
quickMNN.Seurat <- quickMNN.SingleCellExperiment
